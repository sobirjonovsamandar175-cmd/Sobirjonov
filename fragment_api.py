...
_stars_price_cache = {}
_ton_balance_cache = None
_ton_balance_cache_time = 0.0
TON_BALANCE_CACHE_TTL = 8
...
async def _get_ton_balance(force_refresh=False):
    """Fragment walletning haqiqiy TON balansini TONAPI orqali oladi."""
    global _ton_balance_cache, _ton_balance_cache_time

    now = time.time()
    if (
        not force_refresh
        and _ton_balance_cache is not None
        and now - _ton_balance_cache_time < TON_BALANCE_CACHE_TTL
    ):
        return _ton_balance_cache
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"TONAPI balans xatosi: HTTP {response.status_code} — "
            f"{response.text[:300]}"
        )
    ...
    if raw_balance is None:
        raise RuntimeError(f"TONAPI javobida balance topilmadi: {data}")

    balance_ton = round(int(raw_balance) / 1_000_000_000, 4)
    _ton_balance_cache = balance_ton
    _ton_balance_cache_time = now
    return balance_ton
